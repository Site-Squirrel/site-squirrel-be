    ...
    Trip.all.each do |trip|
      ...
      trip.active_days.each do |day|
        availability[:campsites].keys.each do |campsite|
          unless availability[:campsites][campsite][:availabilities][(day.date + 'T00:00:00Z').to_sym] == 'Available'
            next
          end
          ...
          email_text = "A reservation was found on #{day.date} for campsite#{day.site_number} at loop #{day.loop}. Please head to https://www.recreation.gov/camping/campsites/#{campsite} or https://www.recreation.gov/camping/campgrounds/#{trip.campground_id} to reserve your spot."
